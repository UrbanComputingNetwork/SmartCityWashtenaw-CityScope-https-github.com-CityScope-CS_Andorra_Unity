            ...
            { //go through all 'densities' to match Type to Height. Add +1 so #6 (Road could be in. Fix in JSON Needed) 
                if (_Cells.grid[i].type == n)
                {
                    ...
                    { //if this cell is one of the buildings types
                        ...
                        //var _tmpLastScale = _cube.transform.localScale; // get temp scale
                        //var yVelocity = 0.0F;
                        //float _newScale = Mathf.SmoothDamp(_tmpLastScale.y, (_Cells.objects.density[n] * floorHeight), ref yVelocity, 1);
                        //_cube.transform.localScale = new Vector3(cellShrink * cellWorldSize, _newScale, cellShrink * cellWorldSize);
