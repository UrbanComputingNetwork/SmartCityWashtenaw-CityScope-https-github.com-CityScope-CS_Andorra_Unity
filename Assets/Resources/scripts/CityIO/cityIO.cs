...
public class cityIO : MonoBehaviour
{
	...
	void drawTable ()
	{
		...
		for (int i = 0; i < _Cells.grid.Count; i++) {
			...
			_cube.transform.localScale = new Vector3 (cellWorldSize* 0.85f, 0, cellWorldSize * 0.85f);

			//ShowBuildingTypeText (i); /// call if you need type text float 

			for (int n = -1 ; n < _Cells.objects.density.Count + 1; n++) { //go through all 'densities' to match Type to Height. Add +1 so #6 (Road could be in. Fix in JSON Needed) 
				if (_Cells.grid [i].type == n) {
					...
					} else { //if green or other non building type
						_cube.transform.localPosition = new Vector3 (_cube.transform.position.x, -5, _cube.transform.position.z); //hide base plates 
						_cube.transform.localScale = new Vector3 (cellShrink * cellWorldSize * 0.85f, 0.25f, cellShrink * cellWorldSize * 0.85f);
						_cube.AddComponent<NavMeshObstacle> ();
						_cube.GetComponent<NavMeshObstacle> ().carving = true;
	...
	private void ShowBuildingTypeText (int i) //mesh type text metod 
	{
		...
		GameObject textMesh = GameObject.Instantiate (textMeshPrefab, new Vector3 ((_Cells.grid [i].x * cellWorldSize),
			                      100, (_Cells.grid [i].y * cellWorldSize)),
